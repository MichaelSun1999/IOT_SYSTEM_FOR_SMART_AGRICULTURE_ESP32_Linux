...
void HTS221_whole_init(void)
{
    ...
    hts221=i2c_sensor_hts221_init();
    ESP_ERROR_CHECK(hts221_init(hts221, &hts221_config_write));
    ESP_ERROR_CHECK(hts221_get_deviceid(hts221, &hts221_deviceid));
    if(HTS221_WHO_AM_I_VAL!=hts221_deviceid)
    {
        int i=-1;ESP_ERROR_CHECK(i);
    }
    ESP_ERROR_CHECK(hts221_get_config(hts221, &hts221_config_read));
    if((hts221_config_write.avg_h==hts221_config_read.avg_h)&&(hts221_config_write.avg_t==hts221_config_read.avg_t)&&(hts221_config_write.odr==hts221_config_read.odr)&&(hts221_config_write.bdu_status==hts221_config_read.bdu_status)){}
    else
    {
        int i=-1;ESP_ERROR_CHECK(i);
    }
}
static void HTS221_read(void* args)
{
    getting_data = true;
    esp_err_t err;
    HTS221_whole_init();
    while (getting_data) {
        ESP_ERROR_CHECK(hts221_get_humidity(hts221, &humidity));
        ESP_LOGI(HTS221_TAG, "humidity value is: %2.2f %%", (float)humidity / 10);
        ESP_ERROR_CHECK(hts221_get_temperature(hts221, &temperature));
        ESP_LOGI(HTS221_TAG, "temperature value is: %2.2f degC", (float)temperature / 10);
    ...
    while (run_check_button) {
        ...
        if (!new_level && old_level) {
            if (s_route_table_size && !esp_mesh_is_root()) {
                ...
                snprintf(print, sizeof(print),MACSTR, MAC2STR(my_mac));//convert each number and symbol to char
    ...
    while (is_running_tx) 
    {
        if(!esp_mesh_is_root())
        {
            ...
            if(in_list)
            {
                ...
                float tmp1=(float)temperature/10;
                float tmp2=(float)humidity/10;
                asprintf(&print_sensor_data,format,CONFIG_DEVICE_NAME,tmp1,tmp2);
                for(int i=0;i<45;++i)
                {
...
esp_err_t esp_mesh_comm_p2p_start(void)
{
    ...
    if (!is_comm_p2p_started) {
        ...
        xTaskCreate(HTS221_read, "HTS221 read task", 3072, NULL, 5, NULL);
