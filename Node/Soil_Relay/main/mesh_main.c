...
static const char *ADC_TAG = "ADC_main";
...
float test_storage[CONFIG_MESH_ROUTE_TABLE_SIZE]={0};
int start_count=5;
float accumulate=0;
...
static void continuous_adc_init(adc_channel_t *channel, uint8_t channel_num, adc_continuous_handle_t *out_handle)
{
    ...
    adc_continuous_handle_cfg_t adc_config = {
        ...
        .conv_frame_size = EXAMPLE_READ_LEN,//requires:SOC_ADC_DIGI_DATA_BYTES_PER_CONV*N                (SOC_ADC_DIGI_DATA_BYTES_PER_CONV=4)
    };
    ESP_ERROR_CHECK(adc_continuous_new_handle(&adc_config, &handle));

    adc_continuous_config_t dig_cfg = {
        .sample_freq_hz = 20 * 1000,
        .conv_mode = EXAMPLE_ADC_CONV_MODE,//just open SAR ADC1
        .format = EXAMPLE_ADC_OUTPUT_TYPE,//only type2 is valid
    };
...
static void ADC_task(void *arg)
{
    ...
    while(ADC_start)
    {
        ...
        float accumulator=0;int verified=0;
        if (ret == ESP_OK) {
            ...
            accumulator=accumulator/verified;
            ADC_output=accumulator;
            if(start_count>0)
            {
                if(start_count<6)accumulate=accumulate+ADC_output;
                start_count--;
            }
            ESP_LOGI(ADC_TAG, "average:%.2f,  data verified:%d",accumulator,verified);
        }
        else
        {
            ESP_LOGW(ADC_TAG, "Reading failed");
        }
...
static void Relay_task(void *arg)
{
    ...
    while (Relay_on) {
        if(accumulate>0)
        {
            if(ADC_output<accumulate*0.9)
            {
                gpio_set_level(GPIO_OUTPUT_IO_0, 0);
                printf("closed\n");
            }
            if(ADC_output>accumulate*1.1)
            {
    ...
    while (is_running_rx) {
        ...
        if(!esp_mesh_is_root())
        {
            if(pending.toSelf>0)
            {
                ...
                {
                    if(CONFIG_MESH_NODE_FUNCTION_INDEX==2)
                    {
                        if(print_receive[8]=='2')
                        {
                            int j=10;
                            force_humidity=0;
                            while((print_receive[j]>='0')&&(print_receive[j]<='9'))
                            {
                                force_humidity=force_humidity*10+print_receive[j]-'0';
                                j++;
                            }
                            if(force_humidity==0)gpio_set_level(GPIO_OUTPUT_IO_0, 1);
                            if(force_humidity>=4096)gpio_set_level(GPIO_OUTPUT_IO_0, 0);
                            if((force_humidity>0)&&(force_humidity<4096))accumulate=force_humidity;
                            //LED_matrix_on(led_strip,1,force_humidity,force_humidity,force_humidity);
...
esp_err_t esp_mesh_comm_p2p_start(void)
{
    ...
    if (!is_comm_p2p_started) {
        ...
        xTaskCreate(Relay_task, "Relay operation task", 3072, NULL, 5, NULL);
        xTaskCreate(ADC_task, "Soil moisture read task", 3072, NULL, 5, NULL);
